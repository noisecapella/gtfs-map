...
fn shortest_distance_to_segment(point: &Point, line: &(&Point, &Point)) -> f32 {
    ...
    let lon_factor = f32::cos((f32::consts::PI / 180.0) * x0);
    y1 *= lon_factor;
    y2 *= lon_factor;
    y0 *= lon_factor;

    let divisor = f32::sqrt(f32::powi(y2-y1, 2) + f32::powi(x2-x1, 2));
    if divisor == 0. {
        999999.
    } else {
        f32::abs((y2-y1)*x0 - (x2-x1)*y0 + x2*y1 - y2*x1) / divisor
    }
}

pub fn simplify_path<'a>(path: &[Point]) -> Vec<Point> {
