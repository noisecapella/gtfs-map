...
pub fn add_line<'a>(conn: &Connection, startorder: i32, route_ids: &[&str], as_route: &str, agency_id: i32, gtfs_map: &'a GtfsMap, stops_inserted: &mut HashSet<&'a str>, color_override: Option<i32>) -> Result<i32, Error> {
    ...
    let paths: Vec<Vec<Point>> = shapes.iter().map(
        |(shape_id, shapes)| {
            let path: Vec<Point> = shapes.iter().map(|shape| {
                Point { lat: shape.shape_pt_lat, lon: shape.shape_pt_lon }
            }).collect();
            simplify_path(&path)
        }
    ).collect();
    ...
    let routes_added = try!(db::insert_route(conn, as_route, as_route, color_override.unwrap_or(route.route_color), startorder, SUBWAY_AGENCY_ID, &pathblob));
...
pub fn generate_heavy_rail<'a>(connection: &Connection, startorder: i32, gtfs_map: &'a GtfsMap, stops_inserted: &mut HashSet<&'a str>) -> Result<i32, Error> {
...
pub fn generate_commuter_rail<'a>(connection: &Connection, startorder: i32, gtfs_map: &'a GtfsMap, stops_inserted: &mut HashSet<&'a str>) -> Result<i32, Error> {
    let routes_in_order = [
        "CR-Greenbush",
        "CR-Kingston",
        "CR-Middleborough",
